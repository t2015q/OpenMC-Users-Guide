...
xp3=openmc.XPlane(x0= -0.842)
xp4=openmc.XPlane(x0=0.842)
yp5=openmc.YPlane(y0=-0.842)
yp6=openmc.YPlane(y0=0.842)
zp7=openmc.ZPlane(z0= 0.0)
zp8=openmc.ZPlane(z0=91.44)
zp9=openmc.ZPlane(z0= 91.92) #top 0f clad


xp10=openmc.XPlane(x0=40.416)
xp11=openmc.XPlane(x0= 0.0)
xp12=openmc.XPlane(x0=35.364)
yp13=openmc.YPlane(y0=37.048)

yp20=openmc.YPlane(y0=35.364)
...
xp24=openmc.XPlane(x0=70.416 ,boundary_type='vacuum')
xp25=openmc.XPlane(x0=-30,boundary_type='vacuum')
yp26=openmc.YPlane(y0=67.048,boundary_type='vacuum' )
...
u1=openmc.Universe(cells=(cell1,cell2,cell3,cell4,cell5,cell7))

all_water=openmc.Cell(fill=water)
u2=openmc.Universe(cells=[all_water])

#Rectangular Lattices 24*21(x*y)
lattice = openmc.RectLattice()
lattice.lower_left = (0, 0)
lattice.pitch = (1.684,1.684)
lattice.universes = np.tile(u1, (21, 24))  # 24*21
lattice.outer=u2

#Rectangular Lattices 21*1(x*y)
lattice2 = openmc.RectLattice()
lattice2.lower_left = (0,35.364)
lattice2.pitch = (1.684,1.684)
lattice2.universes = np.tile(u1, (1, 21))  # 21*1
lattice2.outer=u2
...
cell10=openmc.Cell()
region10=-xp12 & +xp11 & -yp13 & +yp20 & -zp22 & +zp23
cell10.region= region10
cell10.fill=lattice2


cell9=openmc.Cell()
region9=-xp10 & +xp11 & -yp13 & +yp21 & -zp23 & +zp29
cell9.region= region9
cell9.fill=acrylic

cell11=openmc.Cell()
cell11.region= -xp24 & +xp25 &  -yp26  & +yp27 & -zp28 & +zp30 & ~region8 & ~region9 & ~region10
cell11.fill=water

root=openmc.Universe(cells=(cell8,cell9,cell10,cell11))
